    ...
    <li id="menu-items-">
      <a href="login.php">LOGIN &#9662;</a>
       ...
       <div class="col-sm-3">
        <ul class="level0">
        <h3>DECE</h3><hr>
            <li id="menu-items-24"><a href="ecesyllabus.php">SYLLABUS</a></li><hr>
            <li id="menu-items-25"><a href="ecefaculty.php">FACULTY</a></li><hr>
            ...
            <div class="col-sm-9">
              <h2 align="center">ELECTRONICS AND COMMUNICATION  ENGINEERING SECTION </h2><hr>
              <div class="container">
        <h1 align="center">Results Analysis</h1>
        <?php
        $con = mysqli_connect("localhost", "root", "", "apt");

        if (!$con) {
            die("Connection failed: " . mysqli_connect_error());
        }

        // Display data from the 'placements' table where sem is 0
        $sql1 = "SELECT * FROM results WHERE sem = 1 AND dept='ece'";
        $result1 = mysqli_query($con, $sql1);

        if ($result1) {
            if (mysqli_num_rows($result1) > 0) {
                echo "<h2>FIRST SEMISTER</h2>";
                echo "<table>
                <tr>
                <th>Year</th>
        ...
        // Display data from another table 'other_table' where sem is 1
        $sql2 = "SELECT * FROM results WHERE sem = 3&& dept='ece'";
        $result2 = mysqli_query($con, $sql2);

        if ($result2) {
            if (mysqli_num_rows($result2) > 0) {
                echo "<h2>THIRD SEMISTER</h2>";
                echo "<table>
                <tr>
                <th>Year</th>
                <th>Students Appeared </th>
        ...
        $sql3 = "SELECT * FROM results WHERE sem = 4 AND dept='ece'";
        $result3 = mysqli_query($con, $sql3);

        if ($result3) {
            if (mysqli_num_rows($result3) > 0) {
                echo "<h2>FOURTH SEMISTER</h2>";
                echo "<table>
                <tr>
                <th>Year</th>
                <th>Students Appeared </th>
        ...
        $sql5 = "SELECT * FROM results WHERE sem = 5 AND dept='ece'";
        $result5 = mysqli_query($con, $sql5);

        if ($result5) {
            if (mysqli_num_rows($result5) > 0) {
                echo "<h2>Fifth SEMISTER </h2>";
                echo "<table>
                <tr>
                <th>Year</th>
                <th>Students Appeared </th>
